...
function rawToTiltSwing(e){
  ...
  // 背面垂直:
  // α111 trial: Tiltはalpha/beta/gammaを回転行列へ戻して算出する。
...
function captureFlightRecorder(reason, current){
  ...
  const now = new Date();
  const lines = [
    `ViewCameraAssistant v1α111 Flight Recorder`,
    `${now.toLocaleString()}`,
    `reason: ${reason}`,
    ``,
    `Current`,
...
function clampMeasuredAngleForTarget(target, value){
  // α111: 被写体面だけは±90°で折り返さず、±180°まで連続値として扱う。
