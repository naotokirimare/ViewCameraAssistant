...
function rawToTiltSwing(e){
  ...
  // 背面垂直:
  // α115 trial: Tiltはalpha/beta/gammaを回転行列へ戻して算出する。
...
function captureFlightRecorder(reason, current){
  ...
  const now = new Date();
  const lines = [
    `ViewCameraAssistant v1α115 Flight Recorder`,
    `${now.toLocaleString()}`,
    `reason: ${reason}`,
    ``,
    `Current`,
...
function updateMeasureDebug(mapped){
  ...
  if(typeof focusDebugFor === "function"){
    ...
    if($("dbgScheimXDistance")) $("dbgScheimXDistance").textContent = (typeof fd.scheimX === "number" && typeof fd.scheimY === "number") ? Math.sqrt(fd.scheimX*fd.scheimX + fd.scheimY*fd.scheimY).toFixed(1) : "-";
  ...
  if(typeof opticsDistances === "function"){
    ...
    if($("dbgOptNote")) $("dbgOptNote").textContent = od.distanceNote || "-";
  }


  if(typeof planeCalculationDebugFor === "function"){
    const pc = planeCalculationDebugFor(side);
    if($("dbgPlaneProductRaw")) $("dbgPlaneProductRaw").textContent = fmtDbg(pc.productRaw);
    if($("dbgPlaneProductNorm")) $("dbgPlaneProductNorm").textContent = fmtDbg(pc.productNorm);
    if($("dbgPlaneFocusRaw")) $("dbgPlaneFocusRaw").textContent = fmtDbg(pc.focusRaw);
    if($("dbgPlaneFocusNear")) $("dbgPlaneFocusNear").textContent = fmtDbg(pc.focusNear);
    if($("dbgPlaneDiff")) $("dbgPlaneDiff").textContent = fmtDbg(pc.planeDiffValue);
    if($("dbgTheta1X")) $("dbgTheta1X").textContent = fmtDbg(pc.theta1X);
    ...
    if($("dbgSensorAngle")) $("dbgSensorAngle").textContent = fmtDbg(pc.sensorAngle);
    if($("dbgRelLensSensor")) $("dbgRelLensSensor").textContent = fmtDbg(pc.relLensSensor);
    if($("dbgNearMinus90")) $("dbgNearMinus90").textContent = fmtDbg(pc.nearMinus90);
  }
...
function clampMeasuredAngleForTarget(target, value){
  // α115: 被写体面だけは±90°で折り返さず、±180°まで連続値として扱う。
...
function applyMeasurementSnapshot(showMessage=true){
  // α115: α115ベースに、現在値反映の固定処理だけ追加。
  const wasLive = !!state.sensor.liveApply;
  if(wasLive){
    state.sensor.liveApply = false;
    if($("liveApply")) $("liveApply").checked = false;
    if($("shootLiveToggle")) $("shootLiveToggle").textContent = "リアルタイムOFF";
  }
  applyMeasurementToModel(showMessage);
  if(showMessage && wasLive && $("sensorStatus")){
    $("sensorStatus").innerHTML = "現在値を反映して固定しました。リアルタイム反映はOFFにしました。";
  }
  updateMeasureStatus();
}
