...
function update(){draw2D();updateInfo();draw3D()}function screen(id){document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));$(id).classList.add('active');document.querySelectorAll('.tab').forEach(t=>t.classList.remove('on'));const tab=document.querySelector('.tab[data-go="'+id+'"]');if(tab)tab.classList.add('on');setTimeout(()=>{draw2D();draw3D();drawAxis3D()},80)}function setView(v){state.view=v;$('sideBtn').classList.toggle('on',v==='side');$('topBtn').classList.toggle('on',v==='top');update(); if(typeof syncBroadcast==='function') syncBroadcast()}function setupApp(){document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>screen(b.dataset.go));$('sideBtn').onclick=()=>setView('side');$('topBtn').onclick=()=>setView('top');$('switchView').onclick=()=>{screen('shoot');setView(state.view==='side'?'top':'side')};$('fnum').onchange=()=>{$('fnumCustom').value=$('fnum').value;update()};['focal','bellows','fnumCustom','coc'].forEach(id=>$(id).addEventListener('input',update));$('sensor').onchange=()=>{if($('sensor').value.includes('gfx'))$('coc').value='0.015';if($('sensor').value==='fullframe')$('coc').value='0.020';update()};setup2DDrag(); if(typeof setupHitLineDrag==='function') setupHitLineDrag();setup3D();setupLensSelectors();
  setupMeasurement(); if(typeof setupSync==='function') setupSync(); update();setIsoView()}window.addEventListener('load',setupApp)
